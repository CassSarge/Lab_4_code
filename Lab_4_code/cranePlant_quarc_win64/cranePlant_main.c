...
typedef RT_MODEL_cranePlant_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * cranePlant(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void cranePlant_initialize();
   extern void cranePlant_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(cranePlant_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(cranePlant_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://cranePlant:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: cranePlant -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(cranePlant_DW.HILReadEncoderTimebase_Task,
        (t_clock) cranePlant_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
